...
void skipComment() {
  ...
  readChar();
  //read until get *)
  while (!(charCodes[oldChar] == CHAR_TIMES && charCodes[currentChar] == CHAR_RPAR)) {
    oldChar = currentChar;
    readChar();
  }
  readChar();
...
Token* readConstChar(void) {
  /*printf("read constain\n");*/
  Token *token = makeToken(TK_CHAR, lineNo, colNo);
  
  
  //ignore CHAR_SINGLEQUOTE
  readChar();
  //const string
  int i = 0;
  char *str = token->string;
  ...
  /*
  //read only on character
  token->string[0] = currentChar;
  token->string[1] = '\0';
  
  //ignore other character until get CHAR_SINGLEQUOTE or EOF
...
Token* getToken(void) {
  ...
  default:
    printf("error token: %c", currentChar);
    token = makeToken(TK_NONE, lineNo, colNo);
    error(ERR_INVALIDSYMBOL, lineNo, colNo);
    readChar();
    return token;
